...
| **C** — contêiner | Dossiê Vivo, índices de fontes, logs de passe | Vencimento dos artefatos que indexa | Não se reexecuta: atualiza-se. Está desatualizado quando a Tabela de Estado não reflete o estado real dos artefatos indexados |

**Cabeçalho obrigatório de todo artefato** (YAML no topo do arquivo):
```yaml
artefato: mapa-expectativas
empresa: ANIM3
classe_validade: P                                         # P | F | D | C
finalidade: analise                                        # analise | teste-de-ferramenta
as_of: 2026-08-06
...
4. O campo `finalidade` é **obrigatório em todo artefato**, com default `analise`. Artefato produzido sobre o controle negativo (§10.5) leva `teste-de-ferramenta`. Ausência do campo invalida o artefato (§0, item 6).
5. Artefatos de classe **C** agregam em vez de vencer por conta própria: não têm `valido_ate`, e o `depende_de` aponta para o que indexam. A dependência real está em cada artefato indexado.
...
| ADR-016 | 2026-08-06 | **Extensão do cabeçalho da §4.5**: cria a classe de validade **C (contêiner)** para artefatos que agregam em vez de vencer por conta própria, e torna o campo **`finalidade`** (`analise` \| `teste-de-ferramenta`) obrigatório em todo artefato, com default `analise` | Descoberto ao construir o esqueleto do Dossiê Vivo: o dossiê não vence por preço, por evento nem por data de revisão própria, de modo que P/F/D não o comportam — e, sem classe válida, o §0 item 6 tornava formalmente inválido o artefato central do sistema. O campo `finalidade` já era exigido pela §10.5 para o controle negativo, mas não constava do cabeçalho normativo; padronizá-lo evita que a exigência dependa de o executor lembrar da §10.5 | Ativa |
...
| 0.4 | 2026-08-06 | ADR-016 aprovada: classe de validade **C (contêiner)** acrescentada à §4.5 e campo **`finalidade`** tornado obrigatório no cabeçalho de todo artefato; regras 4 e 5 novas na §4.5. Arquivo passa a se chamar `documento-mestre-novo-loop.md`, conforme a §4.3: cópias com sufixo de versão deixam de existir e o histórico de versões passa a ser o histórico do Git |
